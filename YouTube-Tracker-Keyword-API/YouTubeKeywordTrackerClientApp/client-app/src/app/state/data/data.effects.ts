...
import {
  ...
  ReportsEndpoints,
  UsersEndpoints,
} from '../../services/api-endpoints/endpoints';
...
export class DataEffects {
  ...
  fetch$ = createEffect(() =>
    this.actions$.pipe(
      ofType(fetch),
      mergeMap((action) => {
        return this.dataService.fetch(action.endpoint).pipe(
          map((data) => fetchSuccess({ collection: data as IEntity[] })),
          catchError((error) => {
  ...
  fetchById$ = createEffect(() =>
    this.actions$.pipe(
      ofType(fetchById),
      mergeMap((action) => {
        return this.dataService.fetchById(action.endpoint, action.id).pipe(
          map((data) => fetchByIdSuccess({ data: data as IEntity })),
          catchError((error) => {
  ...
  upload$ = createEffect(() =>
    this.actions$.pipe(
      ofType(upload),
      mergeMap((action) => {
        return this.dataService.upload(action.endpoint, action.formData).pipe(
          map((data) =>
            uploadSuccess({ successMessage: 'Successfully uploaded data' })
          ),
          catchError((error) => {
            console.error('Error occurred while uploading data:', error); // Log the error to the console
            return of(
              uploadError({
                errorMessage: 'Error occurred while uploading data',
              })
            );
          })
        );
  ...
  edit$ = createEffect(() =>
    this.actions$.pipe(
      ofType(edit),
      mergeMap((action) => {
        return this.dataService.edit(action.endpoint, action.formData).pipe(
          map(() =>
            editSuccess({ successMessage: 'Successfully edited data' })
          ),
          catchError(() =>
            of(editError({ errorMessage: 'Error occured while editing data' }))
          )
        );
      })
    )
  ...
  delete$ = createEffect(() =>
    this.actions$.pipe(
      ofType(deleteData),
      mergeMap((action) => {
        return this.dataService.delete(action.endpoint, action.entity).pipe(
          map(() =>
            deleteSuccess({ successMessage: 'Successfully deleted data' })
          ),
          catchError(() =>
            of(
              deleteError({ errorMessage: 'Error occured while deleting data' })
            )
          )
        );
      })
  ...
  deleteSuccess$ = createEffect(
    () =>
      this.actions$.pipe(
        ...
        tap(() => {
          this.toastService.success(DataActionMessages.DeleteSuccess);
          setTimeout(() => {
            window.location.reload();
          }, 1000);
        })
